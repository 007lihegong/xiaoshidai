...
- (void)drawRect:(CGRect)rect
{
    if (self.frame.size.height<216 || self.frame.size.width<320)
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, kScreen_Width, 216);
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonthDay){
        ...
        if (isIOS7) {
            ...
            [self creatValuePointXs:@[[NSString stringWithFormat:@"%d",nian],[NSString stringWithFormat:@"%d",yue],[NSString stringWithFormat:@"%d",ri]]
                          withNames:@[@"年",@"月",@"日"]];
