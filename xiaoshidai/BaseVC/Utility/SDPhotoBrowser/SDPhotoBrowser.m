...
- (void)setupToolbars
{
    ...
    saveButton.backgroundColor = [UIColor DisplayP3Red:0.1f green:0.1f blue:0.1f alpha:0.90f];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    label.backgroundColor = [UIColor DisplayP3Red:0.1f green:0.1f blue:0.1f alpha:0.90f];
...
- (void)setupImageOfImageViewForIndex:(NSInteger)index
{
    ...
    if ([self highQualityImageURLForIndex:index]) {
        if ([[self highQualityImageURLForIndex:index] isKindOfClass:[NSURL class]]) {
            [imageView setImageWithURL:[self highQualityImageURLForIndex:index] placeholderImage:[self placeholderImageForIndex:index]];
        }else if([[self highQualityImageURLForIndex:index] isKindOfClass:[UIImage class]]){
            [imageView setImage:[self placeholderImageForIndex:index]];
        }else{
            PHCachingImageManager *imageManager = [[PHCachingImageManager alloc] init];
            [imageManager requestImageForAsset:(PHAsset *)[self highQualityImageURLForIndex:index] targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeAspectFill options:nil resultHandler:^(UIImage *result, NSDictionary *info) {
...
- (void)showFirstImage
{
    ...
    if ([self.sourceImagesContainerView isKindOfClass:[UICollectionView class]]) {
        ...
        sourceView = [collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:self.currentImageIndex inSection:0]];
