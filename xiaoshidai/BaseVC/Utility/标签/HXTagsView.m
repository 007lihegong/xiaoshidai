...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.frame = frame;
        self.backgroundColor = UIColorWithRGBA(245, 245, 245, 1.0);
        ...
        _tagOriginX = 10.0;
        _tagOriginY = 10.0;
        _tagHorizontalSpace = 8.0;
        _tagVerticalSpace = 8.0;
        _borderColor = UIColorFromRGB(0xd2d2d2);
        _borderWidth = 0.7f;
        _masksToBounds = YES;
        _cornerRadius = 4.0;
        _titleSize = 13;
        ...
        _highlightedBackgroundImage = [self imageWithColor:UIColorWithRGBA(234, 234, 234, 1.0) size:CGSizeMake(1.0, 1.0)];
    }
    return self;
}

//设置标签数据和代理
- (void)setTagAry:(NSArray *)tagAry delegate:(id)delegate withTagArray:(NSArray *)tagsArray{
    @autoreleasepool {
        ...
        for (NSArray *iTags in disposeAry) {
            ...
            for (NSDictionary *tagDic in iTags) {
                ...
                UIButton *__autoreleasing button = [UIButton buttonWithType:UIButtonTypeCustom];
                button.frame = CGRectMake(originX, _tagOriginY+i*(_tagHeight+_tagVerticalSpace), buttonWith, _tagHeight);
                button.layer.borderColor = colorValue(0xdddddd, 1).CGColor;
                ...
                [button setTitleColor:MyBlueColor forState:UIControlStateSelected];
                [button setTitleColor:MyBlueColor forState:UIControlStateDisabled];
                ...
                if (self.flag==0) {
                    button.tag = 100+tagNumber;
                }else{
                    button.tag = 1000+tagNumber;
                }
            ...
            if (_defaultChoice) {
                if ((UIButton *)[self viewWithTag:_selectedTag+100]) {//tag值默认为第一个
                    //[self buttonAction:(UIButton *)[self viewWithTag:_selectedTag+100]];
                    // [(UIButton *)[self viewWithTag:_selectedTag+100] setSelected:YES];
                    UIButton *tempButton;
                    tempButton = (UIButton *)[self viewWithTag:_selectedTag+100] ;
                    [tempButton  setSelected:YES];
                    tempButton.layer.borderColor = MyBlueColor.CGColor;
                    
                }
                if ((UIButton *)[self viewWithTag:_selectedTag+1000]) {//tag值默认为第一个
                    //[self buttonAction:(UIButton *)[self viewWithTag:_selectedTag+100]];
                    // [(UIButton *)[self viewWithTag:_selectedTag+100] setSelected:YES];
                    [(UIButton *)[self viewWithTag:_selectedTag+1000] setSelected:YES];
        ...
        if (self.tagViewMaxY) {
            self.tagViewMaxY(frame.size.height);
        }
    }
}
...
- (void)disposeTags:(NSArray *)ary {
    ...
    for (NSString *tagTitle in ary) {
        ...
        //计算每个tag的宽度
        CGSize contentSize = [tagTitle fdd_sizeWithFont:[UIFont systemFontOfSize:13] constrainedToSize:CGSizeMake(self.frame.size.width-_tagOriginX*2, MAXFLOAT)];
        ...
        if (index == 0) {
            ...
            if (_type == 0) {
                //多行
                if (originX + contentSize.width > SCREEN_WIDTH - _tagOriginX*2) {
...
- (CGFloat)getDisposeTagsViewHeight:(NSArray *)ary {
