...
- (TZAssetModel *)assetModelWithAsset:(id)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    ...
    if ([asset isKindOfClass:[PHAsset class]]) {
        ...
        /// Allow picking video
        if ([[[asset valueForProperty:ALAssetPropertyType] toString] isEqualToString:ALAssetTypeVideo]) {
            type = TZAssetModelMediaTypeVideo;
            NSTimeInterval duration = [[asset valueForProperty:ALAssetPropertyDuration] toInt32];
...
- (void)getOriginalPhotoDataWithAsset:(id)asset completion:(void (^)(NSData *data,NSDictionary *info,BOOL isDegraded))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
        ...
        ALAsset *alAsset = (ALAsset *)asset;
        ALAssetRepresentation *assetRep = [alAsset defaultRepresentation];
        Byte *imageBuffer = (Byte *)malloc((long)assetRep.size);
...
- (void)getVideoOutputPathWithAsset:(id)asset completion:(void (^)(NSString *outputPath))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
        ...
        NSURL *videoURL = [NSURL URLWithString:[[asset valueForProperty:ALAssetPropertyAssetURL] toString]]; // ALAssetPropertyURLs
