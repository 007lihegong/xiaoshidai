...
-(void)setXLabels:(NSArray *)xLabels
{
    ...
    _xLabelWidth = myScrollView.frame.size.width/num+5;
...
-(void)strokeChart
{
    ...
    for (int i=0; i<_yValues.count; i++) {
        ...
        for (int j=0; j<childAry.count; j++) {
            ...
            if ([childAry[j] length]>6) {
                valueString = [NSString stringWithFormat:@"%.2f",[childAry[j] floatValue]];
            }else{
                valueString = childAry[j];
            }
