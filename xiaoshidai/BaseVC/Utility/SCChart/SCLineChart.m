...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    SCChartLabel * label = [[SCChartLabel alloc] initWithFrame:CGRectMake(0.0,chartCavanHeight+UULabelHeight+10, UUYLabelwidth, UULabelHeight)];
    label.text = [NSString stringWithFormat:@"%@",@"单位:万"];
...
//    if (xLabels.count>7) {
//        _xLabelWidth = (self.frame.size.width - UUYLabelwidth)/num+5;
//    }
...
//    if (xLabels.count >7) {
//        self.contentSize = CGSizeMake(self.width+xLabels.count*5, self.height);
//    }
    //画竖线
...
//                [progressline stroke];
        ...
        pathAnimation.duration = childAry.count*0.05;
...
- (void)addPoint:(CGPoint)point index:(NSInteger)index isShow:(BOOL)isHollow value:(CGFloat)value
{
    ...
    if (isHollow) {
        ...
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point.x-UUTagLabelwidth/2.0, point.y-UULabelHeight*2+10, UUTagLabelwidth, UULabelHeight)];
