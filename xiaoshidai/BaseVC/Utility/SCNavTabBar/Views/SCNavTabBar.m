...
- (void)viewConfig
{
   ...
   CGFloat functionButtonX = _showArrowButton ? (SCREEN_WIDTH - ARROW_BUTTON_WIDTH) : SCREEN_WIDTH;
    ...
    _navgationTabBar = [[UIScrollView alloc] initWithFrame:CGRectMake(DOT_COORDINATE, 0, functionButtonX, NAVIGATION_BAR_HEIGHT)];
...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(3.0f, NAV_TAB_BAR_HEIGHT - 3.0f, width - 6.0f, 3.0f)];
    if ([_itemTitles count] == 5) {
        _lineColor == nil ? (_line.backgroundColor = mainHuang) :(_line.backgroundColor = _lineColor);
    }else{
        _lineColor == nil ? (_line.backgroundColor = MyBlueColor) :(_line.backgroundColor = _lineColor);
    }
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        [button setTitleColor:[UIColor blackColor]forState:(UIControlStateNormal)];
         //[UIColor colorWithHexString:@"747474"] forState:UIControlStateNormal];
        button.titleLabel.font = [UIFont systemFontOfSize:15];
        //[UIFont fontWithName:font_Bold size:15];
        ...
        if ((UIButton *)[self viewWithTag:100]) {//tag值默认为第一个
            [self itemPressed:(UIButton *)[self viewWithTag:100]];
        }
    }
...
- (void)itemPressed:(UIButton *)button
{
    if (_flag) {
        for (NSInteger i=0; i<_itemTitles.count; i++) {
            UIButton *temp = (UIButton*)[self viewWithTag:100+i];
        ...
        [UIView animateWithDuration:0.2f animations:^{
            _line.frame = CGRectMake(button.frame.origin.x + 2.0f, _line.frame.origin.y, [_itemsWidth[button.tag-100] floatValue] - 4.0f, _line.frame.size.height);
        }];
...
//        CGSize size = [title sizeWithFont:[UIFont systemFontOfSize:[UIFont systemFontSize]]];
        ...
        //目前只有三个就先这么写
        if ([titles count] == 3) {
            width = [NSNumber numberWithFloat:kScreen_Width/3];
        }else{
            width = [NSNumber numberWithFloat:kScreen_Width/([titles count])];
        }
        [width class];
...
#pragma mark - popView 的高度
                    _popView = [[SCPopView alloc] initWithFrame:CGRectMake(DOT_COORDINATE, 0, SCREEN_WIDTH, self.frame.size.height )];
...
- (CGFloat)updateData
{
    ...
    self.width = [_itemsWidth[0] integerValue] * _itemsWidth.count;
