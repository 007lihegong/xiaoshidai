...
//                       ];
    ...
    for (SerFeeBalance *model in _model.data.balance) {
        if ([model.value floatValue] != 0.00) {
            [items addObject:[SCPieChartDataItem dataItemWithValue:[model.value floatValue] color:SCRandomColor description:model.name]];
        }
    }
    ...
    chartView.descriptionTextFont  = [UIFont fontWithName:@"Avenir-Medium" size:10.0];
